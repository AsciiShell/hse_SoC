...
\begin{document} % конец преамбулы, начало документа
	...
	Т.к. платы с периферийным оборудованием нет, ограничимся симуляцией (рис. \ref{fig:extraimg}). С клавиатурного модуля приходит код необходимого символа, он преобразуется в сигналы азбуки морзе ($shape$), данный модуль работает с помощью конечных автоматов на тактовой частоте ($sck$). Далее под форму этих сигналов преобразуется сигнал на частоте 3КГц ($result$). Этот сигнал и уходит на пьезоизлучатель.
